...
const CameraPermissionInfo: React.FC<CameraPermissionInfoProps> = React.memo(({ onReload }) => {
  ...
  return (
    <div className="camera-permission-info" style={{ maxWidth: 400, margin: '2rem auto', padding: '1.5rem', background: '#232323', borderRadius: 12, color: '#fff', textAlign: 'center' }}>
      ...
      <button onClick={handleReload} style={{ marginTop: '1rem', padding: '0.5rem 1.2rem', borderRadius: 8, border: 'none', background: '#4ecdc4', color: '#222', fontWeight: 600, cursor: 'pointer' }}>
        Seite neu laden
      </button>
    </div>
  );
